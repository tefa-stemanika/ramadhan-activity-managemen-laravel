    ...
    <section>
        <form action="{{ route('kelas.update', $data) }}" method="POST" class="bg-white p-15 space-y-6 rounded-md border-[1.5px] border-[#D9D9D9] max-w-[750px]">
            ...
            <div class="grid grid-cols-4 gap-3 items-center">
                ...
                <input type="text" name="nama" id="nama" {{ $data->nama }} class="col-span-3 bg-white px-5 py-3.5 rounded-md border-[0.5px] border-[#D9D9D9]">
            ...
            <div class="grid grid-cols-4 gap-3 items-center">
                ...
                <select name="username" id="username" class="col-span-3 bg-white px-5 py-3.5 rounded-md border-[0.5px] border-[#D9D9D9]">
                    @foreach (App\Models\User::orderBy('username')->get() as $item)
                        <option value="{{ $item->username }}" @selected($data->username == $item->username)>{{ $item->username }}</option>
                    @endforeach
                </select>
