...
@section('main')
    <section>
            <form action="{{ route('siswa.store') }}" method="post" class="bg-white p-4 md:p-15 space-y-6 rounded-md border-[1.5px] border-[#D9D9D9] max-w-[750px]">
                @csrf
                @method('POST')
                <div class="grid grid-cols-4 gap-3 items-center">
                    <label for="nis" class="col-span-4 lg:col-span-1 text-sm lg:text-lg font-medium">NIS</label>
                    <input type="text" name="nis" id="nis" class="col-span-4 lg:col-span-3 bg-white px-5 py-3.5 rounded-md border-[0.5px] border-[#D9D9D9]">
