...
@section('main')
    <section>
        <div class="flex space-x-8 mb-6 md:mb-12">
            ...
            <p class="text-sm md:text-base font-bold">Tambah Siswa Baru</p>
        </div>
            <form action="{{ route('siswa.store') }}" method="post" class="bg-white p-15 space-y-6 rounded-md border-[1.5px] border-[#D9D9D9] max-w-[750px]">
                @csrf
                @method('POST')
                <div class="grid grid-cols-1 md:grid-cols-4 gap-3 items-center">
                    <label for="nis" class="text-lg font-medium">NIS</label>
                    <input type="text" name="nis" id="nis" class="col-span-3 bg-white px-5 py-3.5 rounded-md border-[0.5px] border-[#D9D9D9]">
